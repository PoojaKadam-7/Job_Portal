...
def recruiter_signup(request):
    ...
    if request.method=='POST':
        ...
        try:
            ...
            Recuiter.objects.create(user=user,email=e,moblie=c,image=i,company=c,gender=g,type='recruiter',status="pending")
            error="no"
        except:
            error="yes"
    d = {'error':error}
...
def admin_home(request):
    ...
    rcount = Recuiter.objects.all().count()
...
def change_status(request,pid):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    error=""
    recruiter = Recuiter.objects.get(id=pid)
    if request.method=="POST":
...
def job_list(request):
    if not request.user.is_authenticated:
        return redirect('recruiter_login')
    user = request.user
    recruiter = Recuiter.objects.get(user=user)
...
def add_job(request):
    ...
    if request.method=='POST':
        ...
        recruiter=Recuiter.objects.get(user=user)
        try:
            Job.objects.create(recruiter=recruiter,start_date=s, end_date=e,title=j, salary=sa,image=l,description=de,experience=ex,location=lo,skill=sk,creationdate=date.today())
            error="no"
        except:
            error="yes"
    d = {'error':error}
...
def user_signup(request):
    ...
    if request.method=='POST':
        ...
        try:
            user=User.objects.create_user(first_name=f, last_name=l,username=e,password=p)
            StudentUser.objects.create(user=user,email=e,moblie=c,image=i,gender=g,type='student')
            error="no"
        except:
            error="yes"
    d = {'error':error}
...
def lastest_jobs(request):
    job = Job.objects.all().order_by('-start_date')
    d = {'job':job}
    return render(request,'lastest_jobs.html',d)
